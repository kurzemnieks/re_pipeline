...
appConfig = re_project.AppConfig(blender=True, houdini=True, maya = False, c4d = True, usd = True)
#appConfig = project.AppConfig(blender=False, houdini=False, maya = False, c4d = False, usd = False)

result = re_project.set_current_root_dir(path)
if not result:
    re_project.create_project(appConfig)
    re_project.create_project_folders()

re_project.update_project_app_config( appConfig )
